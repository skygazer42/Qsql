...
from .utils import (
    ...
    validate_config_path as validate_config_path,
)

api_key: Union[str, None] = None  # API key for legacy hosted service
...
Set the SQL to DataFrame function for QSQL. This is used in the
[`vn.ask(...)`][qsql.ask] function. Instead of setting this directly you can
also use [`vn.connect_to_snowflake(...)`][qsql.connect_to_snowflake] to set
this.

"""

__org: Union[str, None] = None  # Organization name for legacy hosted service
...
def error_deprecation():
    raise Exception("""
The hosted remote runtime is not supported in this branch.

Please initialize QSQL through the local runtime entrypoints under
`src.qsql.local` or use the Flask app defined in `app.py`.
""")
...
def get_api_key(email: str, otp_code: Union[str, None] = None) -> str:
    ...
    **Example:**
    ...
    Login to the legacy hosted API.
