...
from src.utils import setting, Log
...
class Tasker:
    def __init__(self, worker_count: int = 2):
        ...
        os.makedirs(setting.TASKS_DIR, exist_ok=True)
        self._storage_path = os.path.join(setting.TASKS_DIR, "tasks.json")
        self._started = False
                ...
                try:
                    ...
                    if task.cancel_requested:
                        await self._mark_cancelled(
                            task_id, "Task was cancelled before execution"
                        )
                        continue
                    ...
                    try:
                        ...
                        if task.cancel_requested:
                            await self._mark_cancelled(
                                task_id, "Task cancelled during execution"
                            )
                            continue
                        await self._update_task(
                            task_id,
